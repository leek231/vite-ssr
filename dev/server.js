...
const config_1 = require("vite-ssr/config");
...
const createSSRDevHandler = (server, options = {}) => {
    ...
    let { ssr, ...otherOptions } = server.config.inlineConfig;
    ...
    const pluginOptions = (0, config_1.getPluginOptions)(server.config);
    ...
    const handleSsrRequest = async (request, response, next) => {
        ...
        try {
            const entryPoint = options.ssr || (await (0, config_1.getEntryPoint)(server.config, template));
